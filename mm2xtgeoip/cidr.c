...
/*int main(void) {
    ...
    if (inet_ntop(range.addr_family, range.start, start, sizeof(start)) == NULL) {
        puts("Conversion of start address to text failed.");
        return 2;
    }
    
    if (inet_ntop(range.addr_family, range.end, end, sizeof(end)) == NULL) {
        puts("Conversion of end address to text failed.");
        return 3;
    }
    
    if (inet_ntop(range.addr_family, range.mask, mask, sizeof(mask)) == NULL) {
        puts("Conversion of mask to text failed.");
        return 4;
    }
    
    if (inet_ntop(range.addr_family, range.base, base, sizeof(base)) == NULL) {
        puts("Conversion of base address back to text failed.");
        return 5;
    }
    
    if (!unparse_cidr(&range, line, sizeof(line))) {
        puts("Conversion of range back to CIDR failed.");
        return 6;
    }
    
    printf("CIDR: %s\nBase: %s\nStart: %s\nEnd: %s\nMask: %s\nMask bits: %u\n",
        line, base, start, end, mask, range.prefix_length);
